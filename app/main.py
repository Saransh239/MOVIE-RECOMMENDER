...
def results(movie_name):
        ...
        if movie_name not in find_movie['original_title'].unique():
                ...
                recommendations = recommend_movies(title=movie_name, data=find_movie, combine=combine_result, transform=transform_result)
...
@app.route('/get')
def recommend_movies_():
	l=[]
	res = results(str(request.args.get('msg')))
